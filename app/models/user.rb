class User < ActiveRecord::Base
 ...
 def self.find_for_vkontakte_oauth access_token
    if user = User.where(:url => access_token.info.urls.Vkontakte).first
      ...
      User.create!(:friends => access_token.friends.get, :provider => access_token.provider, :url => access_token.info.urls.Vkontakte, :username => access_token.info.name, :nickname => access_token.extra.raw_info.first_name, :email => access_token.extra.raw_info.screen_name+'@vk.com', :password => Devise.friendly_token[0,20]) 
    end
  end
end
